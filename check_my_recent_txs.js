...
async function main() {
  ...
  console.log(`Latest block: ${latest}. Searching last 200 blocks for our transactions...`);

  const promises = [];
  for (let i = latest - 200; i <= latest; i++) {
  ...
  if (sorted.length === 0) {
    console.log("No transactions from our EOA found in the last 200 blocks.");
    return;
  }
