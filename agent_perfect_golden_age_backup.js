...
const GAS_BUY  = ethers.parseUnits('2247', 'gwei');
const GAS_SELL = ethers.parseUnits('225', 'gwei');
const GAS_SELL_END = ethers.parseUnits('155', 'gwei');
...
async function execTrade(exec, sig, label, gasPrice, tokenAddress, isBuy, nonce) {
  ...
  const r = await exec(data, gasPrice, nonce, 445000n);
