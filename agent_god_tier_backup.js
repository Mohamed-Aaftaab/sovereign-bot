...
// ── STRATEGY ──────────────────────────────────────────────────────────────────
let BUY_AMOUNT      = 100n * 10n ** 18n; // Reverted back to 100 USDC because the tradingSafe is strictly provisioned with exactly 100 USDC
...
const GAS_BUY                = 2200n * 10n ** 9n;  // 2200 Gwei (Golden Period max buyout)
const GAS_SELL               = 225n * 10n ** 9n;   // 225 Gwei (Golden Period peak sell)
const GAS_SELL_END           = 50n * 10n ** 9n;    // 50 Gwei (Golden Period end dump)
    ...
    if (game.tradingOpen) {
      ...
      if (inPosition && !inFlight) {
        ...
        if (bal > 0n) {
          ...
          // 1.5. Emergency Stop Loss (-25% from entry)
          else if (!soldPeak && soldEndStep === 0 && avgEntry > 0n && price > 0n && (price * 100n / avgEntry) <= HARD_STOP_PCT) {
            soldEndStep = 1; // Mark as done so we don't double sell
            log(`[HARD STOP] Price crashed below threshold (-25%). Emergency dumping all balance (${ethers.formatUnits(bal, 18)} tokens)`);
            sellTokens(token, bal, 'HARD-STOP', GAS_SELL_END, true)
              .then(success => { if (!success) soldEndStep = 0; });
          }
          // 2. Single phase dump
          else if (remaining <= 15 && soldEndStep === 0) {
            soldEndStep = 1;
            log(`[END SELL] 15s remaining. Selling all remaining balance (${ethers.formatUnits(bal, 18)} tokens)`);
            sellTokens(token, bal, 'END-FINAL', GAS_SELL_END, true)
              .then(success => { if (!success) soldEndStep = 0; });
          }
