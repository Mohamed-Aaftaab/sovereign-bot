...
async function main() {
  ...
  // Fetch a swap signature from the API
  ...
  // Build the tradeViaFactory outer transaction data
  const TRADER_ABI = [
  ...
  const zeroForOne = usdcLower ? true : false; // isBuy = true
  ...
  try {
    console.log("Simulating transaction via provider.call...");
